...
def send_invite_email(recipients, first_name, one_time_password):
    ...
    send_mail(subject, "", email_from, recipients, html_message=content, fail_silently=True)
...
def send_fees_notification(recipients_ids, fees_ids, team_id):
    ...
    email_from = settings.EMAIL_HOST_USER
    send_mail(subject, "", email_from, [recipient.email for recipient in recipients], html_message=content, fail_silently=True)
